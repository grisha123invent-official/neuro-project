...
# Порог и настройки классификатора
...
# Задание 1.1, 2.1 - Фильтрация
# Инициализируем фильтр (например, 1-40 Гц для общей визуализации)
...
def on_eeg(d, eeg: EEGTimedData):
    ...
    # Задание 1.1 - Применяем фильтрацию
...
ax_progress.set_title("BCI Progress (Alpha > Threshold)")
...
def update_plot(_):
    ...
    # Update Impedance Text
    imp_str = "Impedance (kOhm): " + " | ".join([f"{channel_names[i] if i<len(channel_names) else i}: {resistances_values[i]:.1f}" for i in range(CHANNELS)])
