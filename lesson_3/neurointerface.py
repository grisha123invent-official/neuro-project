...
def on_device_list(locator, info, fail_reason):
    ...
    if TARGET_SERIAL is None: chosen = info[0]
    else:
        for dev in info:
...
def update_plot(_):
    ...
    imp_str = "Imp (kOhm): " + " | ".join([f"{channel_names[i] if i<len(channel_names) else i}: {resistances_values[i]:.1f}" for i in range(CHANNELS)])
